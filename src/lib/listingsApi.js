...
function normalizeRow(row, locale = 'en') {
  ...
  return {
    ...
    show_price: row.showPrice === true || row.showPrice === 1,
    show_downpayment: row.showDownpayment === true || row.showDownpayment === 1,
    show_monthly: row.showMonthly === true || row.showMonthly === 1,
    show_full_price: row.showFullPrice === true || row.showFullPrice === 1,
    show_annual: row.showAnnual === true || row.showAnnual === 1,
    show_compound: row.showCompound === true || row.showCompound === 1,
  ...
  try {
    ...
    const list = data.map((row) => {
      ...
      return {
        ...
        mapsUrl: row.mapsUrl ?? row.maps_url ?? '',
        annual_payment: row.annualPayment ?? row.annual_payment ?? '',
        show_price: row.showPrice === true || row.showPrice === 1 || row.show_price === true,
        show_downpayment: row.showDownpayment === true || row.showDownpayment === 1 || row.show_downpayment === true,
        show_monthly: row.showMonthly === true || row.showMonthly === 1 || row.show_monthly === true,
        show_full_price: row.showFullPrice === true || row.showFullPrice === 1 || row.show_full_price === true,
        show_annual: row.showAnnual === true || row.showAnnual === 1 || row.show_annual === true,
        show_compound: row.showCompound === true || row.showCompound === 1 || row.show_compound === true,
      };
    });
    return { data: list, error: null };
  } catch (err) {
...
export async function upsertListing(listing) {
  try {
    const payload = {
      ...
      paymentYears: listing.payment_years ?? listing.paymentYears ?? null,
      paymentDownPct: listing.payment_down_pct ?? listing.paymentDownPct ?? null,
      annualPayment: listing.annual_payment ?? listing.annualPayment ?? '',
      showPrice: listing.show_price !== false,
      showDownpayment: listing.show_downpayment !== false,
      showMonthly: listing.show_monthly !== false,
      showFullPrice: listing.show_full_price === true,
      showAnnual: listing.show_annual === true,
      showCompound: listing.show_compound !== false,
    ...
    return {
      data: {
        ...
        payment_years: payload.paymentYears,
        payment_down_pct: payload.paymentDownPct,
        annual_payment: payload.annualPayment,
        show_price: payload.showPrice,
        show_downpayment: payload.showDownpayment,
        show_monthly: payload.showMonthly,
        show_full_price: payload.showFullPrice,
        show_annual: payload.showAnnual,
        show_compound: payload.showCompound,
